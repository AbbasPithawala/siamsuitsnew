...
import { ArrowForwardIosSharp as ExpandArrowIcon, Logout as LogoutIcon } from "@mui/icons-material";
...
export function AppShell() {
  ...
  const initialExpanded = useMemo(
    () => visibleGroups.find((group) => group.entries.some((entry) => location.pathname.startsWith(entry.path)))?.key ?? false,
              ...
              return (
                ...
                >
                  <AccordionSummary
                    ...
                    id={`${group.key}-header`}
                    expandIcon={<ExpandArrowIcon sx={{ fontSize: "0.8rem", color: isExpanded ? "#fff" : "#242424" }} />}
                  >
                    <GroupIcon sx={{ fontSize: 14, color: isExpanded ? "#fff" : "#242424", mr: 1 }} />
                  ...
                  <StyledAccordionDetails>
                    <ul>
                      {group.entries.map((entry) => (
                        <li key={entry.path}>
                          <NavLink to={entry.path} className={({ isActive }) => (isActive ? "activeList" : "notActive")}>
                            {entry.label}
                          </NavLink>
                        </li>
                      ))}

 ...
 * `features.is_required` correction on existing catalog data, plus the "Shoulder Type"/
 * "Monogram Position" render-slot catalog seed. Matches `db/etl/production.ts`'s pattern
 * (find the "siam-suits" tenant, run each domain, log an `EtlDomainResult` summary) even
 * though this doesn't read from legacy Mongo — see `catalog-render-slots.ts`'s doc comment.
 *
 * `db/seed/index.ts`'s dev seed calls the same two functions against the same tenant, so
 * this script's only real purpose is being independently re-runnable/inspectable in
 * isolation (e.g. right after `etl:production` populates real products for the first time).
 */
async function run(): Promise<void> {
  ...
  for (const r of results.slice(1)) console.log(summarizeDomain(r));

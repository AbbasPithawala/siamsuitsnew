 ...
 *
 * - **Mandatory mode** (`paginationQuerySchema`) — single-purpose endpoints (Customers,
 *   Retailers, Orders, Invoices, Shipping Boxes, Tailors, Users, Roles, Extra Payment
 *   Categories, Order Groups). Nothing else in the app fetches these unfiltered for a
 *   picker, so omitting `page`/`pageSize` still resolves to page 1 of 25, not everything
 *   — that's the actual fix for these lists' unbounded-growth problem.
 * - **Opt-in mode** (`optionalPaginationQuerySchema`) — dual-purpose catalog endpoints
 *   (Products, Super Products, Features with no `productId`, Measurement Definitions,
 *   Processes) also consumed unfiltered by the order-builder's own pickers. Omitting
 *   `page`/`pageSize` must keep returning the exact same full unpaginated `{ data: [...] }`
 *   shape as before this convention existed — pagination only activates once the caller
 *   explicitly sends a `page` and/or `pageSize`.
 *
